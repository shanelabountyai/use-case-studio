...
**Corpus ingestion & indexing:** Export help-centre articles, resolved Zendesk tickets and the macro library to a staging document store → Curate macros with the support content owner: mark current / needs-rewrite / retired → Convert to text, strip boilerplate and navigation, chunk on semantic boundaries → Run redaction on ticket-derived content (names, order detail, card-last-four) and record removed field classes → Attach metadata: source type, product area, locale, last-reviewed date, staleness flag, question type where labelled → Embed and write to the vector index alongside a lexical index; exclude retired/stale-flagged content by filter → Scheduled re-index on each source's update cadence; log content gaps from retrieval misses
**Draft generation at ticket open:** Agent opens or is assigned a ticket; console sends subject, body and relevant thread context → Redaction pass masks PII with stable placeholders before the trust boundary → Hybrid retrieval with metadata filters excluding stale content; top passages returned with identifiers → Versioned prompt assembled: house style, refusal/escalation rules, retrieved passages, required citation format → Model generates draft with inline citations → Deterministic post-checks: citations resolve to retrieved chunks, no unmasked sensitive patterns, no placeholder leakage → Placeholders re-hydrated locally; draft rendered into the agent editor with cited articles, or a 'no confident draft' state is shown
**Human review & acceptance-bar measurement:** Agent reads draft, checks citations, edits or discards → Agent sends; there is no code path from generation to outbound send → Telemetry records draft text, final sent text, reviewer rubric action, prompt/model/retrieval versions and retrieved chunk ids → Edit-similarity computed automatically; human rubric tiering applied to the sampled tickets → Failures classified into the taxonomy (retrieval miss, stale grounding, reasoning, tone, formatting, safety) and added to the golden set → Weekly dashboard: minor-edit rate on the sample, CSAT vs pre-pilot baseline, reviewer agreement, refusal rate, proximity to rollback trigger
...
**Fabrication scan**
- `A separate tab or external tool will not be used under time pressure and will silently kill adoption — and adoption is what the acceptance bar measures.` → must-label
- `Public content carries no PII.` → must-label
- `volume is large but the corpus is bounded reference material, so index size is manageable with a managed vector store or a self-hosted index` → must-label
- `Auto-send is not implemented — there is no code path from generation to outbound message.` → ok
- `~2 weeks (estimate)` → ok

**Consistency issues**
- Milestone durations sum to ~17 weeks (2+3+3+3+2+4) but resourcing is stated as 'one builder for a quarter' (~13 weeks), and the Delivery section asserts 'Definition of done for the quarter: the bar has been measured on live traffic over four weeks'. The schedule cannot fit the stated budget as written; the assumption note ('estimates... not commitments') acknowledges revisability but never reconciles the overrun.
- Phase 3 exit criterion says 'offline rubric scoring reported against the ≥80% minor-edit definition', but the acceptance bar is defined as drafts *sent* with only minor edits by agents — offline scoring on historical tickets has no send action and can at best be a proxy. The plan never labels it as a proxy or states the acceptance threshold used offline.
- A pre-pilot CSAT baseline is listed only as an assumption ('exists or can be established'), yet Phase 5 arms a rollback trigger on 'CSAT-vs-baseline' and Phase 6 measures against it. No milestone owns establishing the baseline.
- Integration notes require the telemetry contract to be 'Validate[d] ... before model work begins', but console integration and the telemetry stub are scheduled in Phase 4, after Phase 3 model/prompt work; the assumptions list also defers stub validation to Phase 4.
...
**Verdict integrity:** pass — Plan carries BUILD, matching the grounding verdict, and does not argue for a rosier reading — it preserves the risk score's implications (oversight as launch condition, PII redaction, no auto-send) and pre-commits a stop decision in Phase 6.

---

### Provenance

- Engine outputs hash: `sha256:78cc166b8dc71fd5`
