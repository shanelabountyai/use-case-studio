...
- **Branch when a push would ship something you can't eyeball afterwards** (amended Aug 14, 2026): anything that changes plan generation — planner/critic prompts, the roster version, invariants, the engine's scoring — or anything client-facing in a generated deliverable. Everything else goes straight to main as above. The rule isn't ceremony: a push deploys, and a bad plan reaches a client as a document rather than as a stack trace. Verify a generation change with `BK_LIVE=1` on the branch before merging. Sibling build repos (`builds/*`) have no deploy and no CI — commit direct, don't branch.
